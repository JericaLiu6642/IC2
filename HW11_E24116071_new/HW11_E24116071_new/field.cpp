...
#define NPC 303 //單純NPCC
#define THREE 305 //用錢換exp 1000元可升等
//存檔事件
#define SAVE 400
...
void Field::display(void) const
{
	...
	for (int i = current_position_x - 3; i <= current_position_x + 3; i++) {
		for (int j = current_position_y - 3; j <= current_position_y + 3; j++) {
			...
			else if (map_data[i][j] == CHARM || map_data[i][j] == TRANS || map_data[i][j] == POTION) {
				SetColor(176);
				cout << "C ";
				SetColor();
			}
...
void Field::Events(int n)
{
	...
	case BATTLE_V:
		...
		monPoint.push_back(new JWMonster("Lord Voldemort", 250 + 10 * player->getLevel(), 150 + 5 * player->getLevel()
			, 1000, 1000, 200 + 15 * player->getLevel(), 100));
...
void Field::Potion()
{
	...
	cout << "Southwest." << endl;
	system("pause");
	cout << "And yet, the 'northeast' is worth exploring, too." << endl;
	system("pause");
	system("cls");
	map_data[current_position_x][current_position_y] = 0;
	display();
...
void Field::Three()
{
	...
	cout << "[The man]  I can use my magic to make you level up by 1, but you have to give me $10000 as a return." << endl;
	system("pause");
